...
const LectureTab = () => {
  ...
  const fileChangeHandler = async (e) => {
    ...
    if (file) {
      ...
      try {
        const res = await axios.post(`https://coursemacy.onrender.com/media/upload-video`, formData, {
  ...
  const editLectureHandler = async (e) => {
    ...
    try {
        setLoading(true)
        const res = await axios.post(`https://coursemacy.onrender.com/course/${courseId}/lecture/${lectureId}`, data, {
            headers: {
        ...
        if(res.data.success){
            dispatch(setLecture([
              ...lecture.filter(l => l._id !== lectureId), // remove the old lecture
              res.data.lecture, // add the updated one
            ]));            
            toast.success(res.data.message)
        } 
    } catch (error) {
  ...
  const removeLectureHandler = async (e) => {
    ...
    try {
        ...
        const res = await axios.delete(`https://coursemacy.onrender.com/course/lecture/${lectureId}`, 
        {withCredentials:true})
        if(res.data.success){
      ...
      <CardContent>
        ...
        <div className="mt-4">
          <Button disabled={loading} onClick={editLectureHandler} className="bg-gray-800 hover:bg-gray-800">
            {
                loading ? <><Loader2 className="me-1 w-4 h-4 animate-spin"/>Please wait</> : "Update Lecture"
            }
          </Button>
        </div>
      </CardContent>
