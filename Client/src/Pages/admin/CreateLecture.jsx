...
const CreateLecture = () => {
  ...
  const createLectureHandler = async () => {
    try {
        setLoading(true)
        const res = await axios.post(`https://coursemacy.onrender.com/course/${params?.courseId}/lecture`,{lectureTitle},{
          headers:{
  ...
  useEffect(()=>{
    const getLectures = async()=>{
      try {
          const res = await axios.get(`https://coursemacy.onrender.com/course/${params.courseId}/lecture`,
          {withCredentials:true})
          if(res.data.success){
  ...
  return (
    <div className="p-4 md:p-10 md:pr-20 h-screen">
      ...
      <p>
        Have a valuable lesson to share? Effortlessly upload and present your
        expertise in a focused format. Perfect for standalone lessons,
        supplementary content, or building blocks for larger courses. Start
      ...
      <div className="mt-10">
        {
          lecture?.map((lecture, index)=>{
            return <div key={index} className="flex items-center justify-between bg-[#F7F9FA] px-4 py-2 rounder-md my-2">
